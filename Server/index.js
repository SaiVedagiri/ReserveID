...
//   let count = 0;
//   await database.ref('inStore').once('value', snapshot => {
//     if (snapshot.val().numOfPeople == 1) {
//       count += snapshot.val().numOfPeople;
//       database.ref('users').orderByChild('ready').equalTo('true').once('value', snapshot2 => {
//         snapshot2.forEach(function(childSnapshot) {
//           if(childSnapshot.val().ready == "true") {
//             count++;
//           }
//         })
//         if (count < 2) {
//           database.ref('scheduled').once('value', snapshots => {
//             snapshots.forEach(function(childSnapshot) {
//               if (childSnapshot.key != 'numOfPeople') {
//                 let date = new Date()
//                 if (Date.parse(date) > Date.parse(childSnapshot.val().date)) {
//                   database.ref(`users/${childSnapshot.val().user}`).once('value', snapshot1 => {
//                     if (snapshot1.val().ready != "true") {
//                       database.ref(`users/${childSnapshot.val().user}`).update({
//                         ready: "true"
//                       })
//                       scheduled = true
//                       wss.clients.forEach(function each(client) {
//                         if (client.readyState === WebSocket.OPEN) {
//                           client.send(`${childSnapshot.val().user}`);
//                         }
//                       });
//                     }
//                   })
...
//             })
//           })
//           database.ref('queue').once('value', snapshot3 => {
//             snapshot3.forEach(function(childSnapshot2) {
//               let date = new Date; 
//               let position = childSnapshot2.val().position;
//               if (!scheduled) {
//                 if(position == 1) {
//                   database.ref(`users/${childSnapshot2.val().user}`).update({
//                     ready: 'true'
//                   })
//                   wss.clients.forEach(function each(client) {
//                     if (client.readyState === WebSocket.OPEN) {
//                       client.send(`${childSnapshot2.val().user}`);
//                     }
//                   });
//                 }
//               }
